...
/**
 * Identifies an element and returns a human-readable name + path + tag
 */
export function identifyElement(target: HTMLElement): { name: string; path: string } {
  ...
  // SVG elements
  if (['circle', 'g', 'line', 'path', 'rect'].includes(tag)) {
  ...
  if (['article', 'aside', 'div', 'footer', 'header', 'main', 'nav', 'section'].includes(tag)) {
  ...
  const siblingIds = siblings.slice(0, 4).map((sib) => {
    ...
    if (typeof className === 'string' && className) {
      const meaningful = className
        .split(/\s+/)
        .map((c) => c.replace(/_[a-z0-9]{5}.*$/i, '')) // Remove module hashes
...
export function getElementClasses(target: HTMLElement): string {
  ...
  const classes = className
    ...
    .map((c) => {
      ...
      const match = c.match(/^([a-z][\w-]*?)(?:_[a-z0-9]{5,})?$/i)
