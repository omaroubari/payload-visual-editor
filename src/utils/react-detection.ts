...
interface ReactFiber {
  ...
  tag: number
  type: ComponentType | null | string
}
...
function resolveConfig(config?: ReactDetectionConfig): ResolvedConfig {
  ...
  if (config?.skipExact) {
    ...
    skipExact = new Set([...additional, ...DEFAULT_SKIP_EXACT])
  }
    ...
    if (current.className && typeof current.className === 'string') {
      current.className.split(/\s+/).forEach((cls) => {
        if (cls.length > 1) {
          ...
          const normalized = cls.replace(/_[a-z0-9]{5}.*$/i, '').toLowerCase()
...
const componentCacheAllRef = { map: componentCacheAll }
...
function getReactFiberKey(element: HTMLElement): null | string {
...
function getFiberFromElement(element: HTMLElement): null | ReactFiber {
  const key = getReactFiberKey(element)
  if (!key) {return null}
  return (element as unknown as Record<string, unknown>)[key] as null | ReactFiber
}

function getComponentNameFromType(type: ComponentType | null): null | string {
  if (!type) {return null}
...
function getComponentNameFromFiber(fiber: ReactFiber): null | string {
  const { type, elementType, tag } = fiber
...
export function getReactComponentName(
  ...
  if (!isReactPage()) {
    const result: ReactComponentInfo = { components: [], path: null }
    if (useCache) {
  ...
  const result: ReactComponentInfo = { components, path }
  if (useCache) {
    componentCacheAllRef.map.set(element, result)
  }
  return result
