...
type MockFiber = {
  elementType: { displayName: string; name: string }
...
function createElementWithFiber(fiber: MockFiber, options: { className?: string } = {}) {
...
describe('react-detection', () => {
  ...
  describe('getReactComponentName', () => {
    ...
    it('uses displayName when available', () => {
      const fiber = {
        type: { name: 'InternalName', displayName: 'MyDisplayName' },
        elementType: { name: 'InternalName', displayName: 'MyDisplayName' },
        return: null,
        tag: 0,
      }
  ...
  describe('filter modes', () => {
    ...
    it('mode: all includes more components', () => {
      ...
      expect(allResult.components.length).toBeGreaterThanOrEqual(filteredResult.components.length)
    })
  })
