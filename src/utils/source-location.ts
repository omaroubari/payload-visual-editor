...
export function detectReactApp(): {
  ...
  if (typeof window === 'undefined') {
    return { isProduction: true, isReact: false }
  }
 ...
 */
export function getFiberFromElement(element: HTMLElement): null | ReactFiber {
...
function findDebugSource(
  fiber: ReactFiber,
  maxDepth = 50,
): { componentName: null | string; source: ReactFiber['_debugSource'] } | null {
  let current: null | ReactFiber | undefined = fiber
  let depth = 0
...
const sourceProbeCache = new Map<Function, null | SourceLocation>()
...
function unwrapComponentType(fiber: ReactFiber): Function | null {
  ...
  const elementType = fiber.elementType as null | Record<string, unknown> | undefined
...
function parseComponentFrame(
  stack: string,
): { column?: number; fileName: string; line: number } | null {
 ...
 */
function probeComponentSource(fiber: ReactFiber): null | SourceLocation {
  ...
  let result: null | SourceLocation = null
 ...
 */
function probeSourceWalk(fiber: ReactFiber, maxDepth = 15): null | SourceLocation {
  let current: null | ReactFiber | undefined = fiber
  let depth = 0
...
export function getSourceLocation(element: HTMLElement): SourceLocationResult {
  ...
  if (probed) {
    return { found: true, isProduction: false, isReactApp: true, source: probed }
  }
...
export function formatSourceLocation(
  ...
  const { columnNumber, fileName, lineNumber } = source
